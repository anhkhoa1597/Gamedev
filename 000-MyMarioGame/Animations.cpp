...
void CAnimations::Add(LPCWSTR filePath, int animationId, int length, int defaultTime)
{
	...
	for (int i = 0; i < length; i++) { 
		spriteId += i;
		sprites->Add(spriteId, i * spriteWidth, 0, ((i + 1) * spriteWidth) - 2, spriteHeight - 2, texture);
		ani->Add(spriteId);
	}
...
void CAnimations::Add(LPCWSTR filePath, int animationId, int length, int spriteWidth, int spriteHeight, int defaultTime)
{
	//every animation is a texture
	int textureId = animationId;
